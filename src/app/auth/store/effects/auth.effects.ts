...
import { AuthService, LocalStorageService } from '@app/auth/services';
  ...
  loginUser$ = this.actions$.pipe(
    ...
    switchMap(action => {
      ...
      return this.authService.logIn(name, password).pipe(
        tap(token => this.localStorageService.setItem(environment.auth, { token })),
  ...
  logoutUser$ = this.actions$.pipe(
    ...
    switchMap(() => {
      return this.authService.logOut().pipe(
        tap(() => this.localStorageService.setItem(environment.auth, { token: null })),
